...
'''
class ExpertSurpriseWorker(Worker):
    """Initialize a worker.
    Args:
    ...
    def step_episode(self):
        ...
        if self._eps_length < self._max_episode_length:
            reg_input_shape = self.env.action_space.shape[0]+ self.env.observation_space.shape[0]
            reg_output_shape = self.env.observation_space.shape[0]
            self.teacher_regressor = Regressor(reg_input_shape, reg_output_shape, 32)
            self.student_regressor =  Regressor(reg_input_shape, reg_output_shape, 32)
            a, agent_info = self.agent.get_surprise_action(self._prev_obs, self.teacher_regressor, self.student_regressor, self.student, self.student_sampler, self.env, self.eta0)
    ...
    def collect_episode(self):
        ...
        if self.surprisal_bonus == True: 
            ...
            self.teacher_regressor.fit(self.state_action, self.new_states)
...
def identity_function(value):
    ...
    return value
'''
class ExpertSurpriseWorkerFactory:
    ...
    def __init__(
            ...
            *,  # Require passing by keyword.
            ...
            worker_class=ExpertSurpriseWorker,
            worker_args=None):
