...
Transition = namedtuple("Transition", ("state", "action", "reward", "new_state", "log_prob", "rtg"))
...
class TeacherReplayBuffer(object):
    ...
    def insert(self, rollouts):
        for j in range(len(rollouts.state)):
            ...
            self.memory.rtg.append(rollouts.rtg[j])
    ...
    def sample_batch(self, batch_size):
        ...
        for k in range(len(indxs)):
            ...
            samples.rtg.append(self.memory.rtg[indxs[k]])
...
class Training():
    ...
    def plot(self):
        ...
        #ax1[0].plot(self.its, self.surprise, label = "Surprise Bonus")
        ax1[0].plot(self.its, self.t_rew, label = "Teacher reward")
        ax1[0].plot(self.its, self.s_rew, label = "Student reward")
        ax1[0].set_ylabel("Average Validation Reward")
        ax1[1].plot(self.its, self.t_surprise, label = "Teacher") 
        ax1[1].plot(self.its, self.s_surprise, label = "Student")

        ax1[1].set_ylabel("Average Surpsise")
        ...
        ax2[0].set_ylabel('Position on x-axis')
        ax2[1].hist(vel, bins = 200)
        ax2[1].set_ylabel('Velocity')
        ax2[2].hist(self.actions_vis, bins = 3)
        ax2[2].set_ylabel('Actions')
        fig2.suptitle(" Frequency of States and Actions ")

        fig3, ax3 = plt.subplots(2,1)
        ax3[0].plot(self.its, self.ta_loss, label = "Teacher")
        ax3[0].plot(self.its, self.sa_loss, label = "Student")
        ax3[0].set_ylabel("Actor Loss")
        ax3[1].plot(self.its, self.tc_loss, label = "Teacher")
        ax3[1].plot(self.its, self.sc_loss, label = "Student")
        ax3[1].set_ylabel("Critic Loss")
    ...
    def surprise_reward(self,teacher_reward, t_prob, student_reward, student_policy, states, actions, new_states, t_surprise_model, s_surprise_model): 
        with torch.no_grad(): 
            ...
            surprise_reward = teacher_reward.reshape([teacher_reward.shape[0], 1]) - eta1*t_prob.reshape([t_prob.shape[0], 1]) + eta2*(t_prob.reshape([t_prob.shape[0], 1]) -s_prob.reshape([s_prob.shape[0], 1]))
    ...
    def train_loop(self, env, teacher_model, student_model): 
        ...
        for i in range(self.iters):
            ...
            rtg = torch.from_numpy(np.asarray(rollouts.rtg)).float()
            ...
            rew_sup = self.surprise_reward(rtg, probs, student_rewards, student_model, states, actions, new_states, t_surprise_prob, s_surprise_prob)        
            #rew_sup = rewards
            ...
            self.ta_loss = np.append(self.ta_loss, al.detach().numpy())
            self.tc_loss = np.append(self.tc_loss, cl.detach().numpy())
            ...
            self.sa_loss = np.append(self.sa_loss, al.detach().numpy())
            self.sc_loss = np.append(self.sc_loss, cl.detach().numpy())  
            
            #test student network -- rollouts? 
            #update student reward 
            student_rewards, _ = torch.tensor(student_model.validate(env))
            t_reward, _ = torch.tensor(teacher_model.validate(env))
            if i % 10 == 0: 
                print("Avg. Teacher extrinsic reward / batch: " + str((rewards.sum()/self.timesteps_per_batch).item()))
                print("teacher surprise bonus reward: " + str((rew_sup.sum()/self.timesteps_per_batch).item()))
                print("average student reward:" + str((student_rewards.sum()/self.timesteps_per_batch).item()))
            
            self.its = np.append(self.its, i)
        
           
            self.surprise = np.append(self.surprise, (rew_sup.sum()/self.timesteps_per_batch).item())
            
            self.t_rew = np.append(self.t_rew, (t_reward.sum()/self.timesteps_per_batch).item())
            self.s_rew = np.append(self.s_rew, (student_rewards.sum()/self.timesteps_per_batch).item())
